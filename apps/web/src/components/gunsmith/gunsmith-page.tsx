...
export function GunsmithPage({ initialQuery = "" }: { initialQuery?: string }) {
  ...
  return <ServiceShell title="三角洲改枪码" description="按枪械检索已人工整理的改枪码；复制后请在游戏内自行确认导入结果。" initialQuery={query} onSearch={submitSearch} backHref="/accounts" backLabel="返回账号列表">
    ...
    {games.status === "loading" ? <section className="gunsmith-state" role="status">正在读取改枪码目录…</section> : gameUnavailable ? <section className="gunsmith-state" role="alert"><h2>改枪码目录暂不可用</h2><p>{messageFor(games.status === "error" ? new Error() : null)}</p><button type="button" className="button secondary" onClick={retry}><RotateCw size={15} />重试</button></section> : firearms.status === "loading" ? <section className="gunsmith-state" role="status">正在读取枪械…</section> : firearms.status === "error" ? <section className="gunsmith-state" role="alert"><h2>枪械列表加载失败</h2><p>请稍后重试。</p><button type="button" className="button secondary" onClick={() => loadFirearms()}><RotateCw size={15} />重试</button></section> :
