...
export interface DeltaSectionProps {
  onActionNotice: (title: string, message: string) => void;
  accounts?: AccountCardData[];
  supplyState?: SupplyState;
  searchQuery?: string;
  onResetSearch?: () => void;
}
export function DeltaSection({ onActionNotice, accounts = [], supplyState = "unavailable", searchQuery = "", onResetSearch }: DeltaSectionProps) {
  return <section id="delta-section" className="portal-width delta-section game-section" aria-label="三角洲行动专区">
    <div className="game-row">
      <GameIdentity game="delta"><button className="button delta-tool" onClick={() => onActionNotice("三角洲改枪码", "当前没有可展示的改枪码。请稍后再来。")}><Crosshair size={16} />改枪码</button></GameIdentity>
      <div className="delta-supply" id="account-list" tabIndex={-1}>
        ...
        {supplyState === "loading" ? <div role="status" aria-label="正在加载账号" className="account-grid"><AccountCardSkeleton /><AccountCardSkeleton /><AccountCardSkeleton /></div> :
          supplyState === "ready" && accounts.length > 0 ? <div className="account-grid">{accounts.map((data) => <AccountCard key={data.id} data={data} onActionNotice={onActionNotice} />)}</div> :
          <AccountCardEmpty {...(supplyState === "ready" ? searchQuery.trim() ? { message: "未找到匹配账号", description: "试试其他账号编号或名称，也可以清空搜索查看全部。", onReset: onResetSearch } : { message: "暂无可选账号", description: "当前没有可展示的号源，请稍后再来。" } : supplyState === "error" ? { message: "账号列表加载失败", description: "请稍后重新访问。" } : {})} />}
      </div>
    </div>
  </section>;
}
