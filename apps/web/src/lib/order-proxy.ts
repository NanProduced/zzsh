...
export async function forwardOrderRequest(
  request: Request,
  context: { params: Promise<{ path?: string[] }> },
  resource: "orders" | "order-confirmations" | "orders-v2" = "orders",
): Promise<Response> {
  ...
  if (
    (resource !== "orders" && (method !== "POST" || path !== "/")) ||
  ...
  try {
    upstream = await fetch(
      new URL(
        "/api/bff/user/" + resource + (resource === "orders" ? path : "") + new URL(request.url).search,
        apiOrigin,
      ),
      {
        method,
...
export async function GET(
  ...
  return forwardOrderRequest(request, context);
}
