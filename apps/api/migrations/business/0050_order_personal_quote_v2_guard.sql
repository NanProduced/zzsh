-- TR-B3B-1: keep the 0041 personal-order guard and extend its accepted
-- persisted snapshot contract to the already-produced personal-quote-v2 shape.
    ...
    IF personal_schema='personal-quote-v2' THEN
      ...
      IF key_count <> 12 OR EXISTS (
        ...
        WHERE NOT (object_key.key = ANY (ARRAY[
          ...
          'fullPayoutDeclaration','ruleRefs','membership','guarantee','funding','quote'
        ]::text[]))
      ) THEN
        RAISE EXCEPTION 'invalid personal quote v2 shape' USING ERRCODE='40001';
      END IF;
      IF personal_doc->>'ownerUserId' IS DISTINCT FROM NEW.owner_user_id
      ...
      quote_doc := personal_doc->'quote';
      IF jsonb_typeof(quote_doc) IS DISTINCT FROM 'object'
        OR quote_doc->>'priceVersionId' IS DISTINCT FROM rule_refs_doc->>'priceVersionId'
        OR quote_doc->>'ruleReleaseId' IS DISTINCT FROM NEW.rule_release_id
        OR quote_doc->>'termSeconds' IS DISTINCT FROM NEW.term_seconds::text THEN
      ...
      IF NOT EXISTS (
        SELECT 1
         ...
         WHERE v.id=NEW.listing_version_id
           ...
           AND v.payload#>>'{quoteValues,priceVersionId}' IS NOT DISTINCT FROM quote_doc->>'priceVersionId'
           AND v.payload#>>'{quoteValues,termSeconds}' IS NOT DISTINCT FROM quote_doc->>'termSeconds'
