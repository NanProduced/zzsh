...
import { adminRequest, friendlyError, hasPermission, type SessionSnapshot } from "../api";
...
type Presence = { adminUserId: string; availability: "OFF_DUTY" | "AVAILABLE" | "PAUSED"; connectionState: string; activeLoad: number; capacity: number };
...
export function ImSupportView({ snapshot, preview = false }: { snapshot: Extract<SessionSnapshot, { authenticated: true }>; preview?: boolean }) {
  ...
  const [presence, setPresence] = useState<Presence>({ adminUserId: snapshot.adminUserId, availability: "OFF_DUTY", connectionState: "DISCONNECTED", activeLoad: 0, capacity: 3 });
  ...
  const operatorKey = `${snapshot.adminUserId}:${snapshot.session.id}:${[...snapshot.permissions].sort().join("\u0000")}`;
  ...
  const writePresence = useCallback((availability: Presence["availability"], state: ReturnType<typeof connectionForPresence>) => {
    ...
    const operation = presenceWriteRef.current.then(async () => {
      ...
      const value = await adminRequest<Presence>("/im/presence", { availability, connectionState: state }, "PUT");
      if (operatorRef.current === key && operatorGenerationRef.current === generation) setPresence(value);
      return value;
    });
  ...
  useEffect(() => {
    ...
    setDrafts({});
    setSelectedId(null);
    setPresence({ adminUserId: snapshot.adminUserId, availability: "OFF_DUTY", connectionState: "DISCONNECTED", activeLoad: 0, capacity: 3 });
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (presenceResult.status === "fulfilled" && presenceResult.value) { setPresence(presenceResult.value); availabilityRef.current = presenceResult.value.availability; }
  ...
  const canSend = preview || Boolean(selected?.state === "ACTIVE" && selected.conversationId && selected.messageScopeState === "READY" && clientRef.current && connection === "CONNECTED");
  ...
  const close = async () => {
    if (!selected || selected.state !== "ACTIVE" || selected.assignedAdmin?.id !== snapshot.adminUserId) return;
    const generation = operatorGenerationRef.current;
    const operator = operatorKey;
    const consultationId = selected.id;
    setBusy(true); setNotice(undefined);
  ...
  const send = async (event: FormEvent<HTMLFormElement>) => {
    ...
    } catch (cause) { if (generation === operatorGenerationRef.current && operatorRef.current === operator) setNotice(cause instanceof Error ? cause.message : "消息发送失败，请重试。"); } finally { if (generation === operatorGenerationRef.current && operatorRef.current === operator) setBusy(false); }
  };
  ...
  return <section className="im-support-view" data-preview={preview} aria-label="客服工作台">
    ...
    <div className="im-support-grid">
      ...
      <aside className="im-support-inspector" aria-label="用户与咨询上下文">
        ...
        <div className="im-support-actions">{!preview && selected?.state === "WAITING" && !selected.assignedAdmin && canAccept ? <button type="button" onClick={() => void claim()} disabled={busy || connection !== "CONNECTED"}>接入会话</button> : null}{!preview && selected?.state === "ACTIVE" && selected?.messageScopeState !== "FAILED" && selected.assignedAdmin?.id === snapshot.adminUserId ? <button type="button" onClick={() => void close()} disabled={busy}>结束会话</button> : null}<button type="button" disabled title="转交目标列表和权限策略尚未在本轮开放">转交会话</button><p>接入、结束和转交均由服务端检查权限并记录审计事件。</p></div>
      </aside>
    </div>
  </section>;
}
