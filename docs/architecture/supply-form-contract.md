...
当前为数据接入层；正式页面尚未接入。API负责报价、资格和对象范围，前端按下列分组自由跳转，不建立另一套发布状态机。
...
| inventory | publishing-catalog.items的稳定itemId、基础币/颗/件整数数量文本 | inputScale=0；必填填数量或0，空值留草稿；未定价blockers阻止提交，不补价格 |
...
| media | 明确ACCOUNT_DISPLAY与ACCOUNT_EVIDENCE；上传意图→原始字节上传→assetId绑定 | 写入只带assetId/position；SavedDeclaration额外purpose/byteHash为只读，editableDeclaration裁成可写字段 |
