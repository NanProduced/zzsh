...
用户端表单使用 `/api/auth/user/sign-in/identifier`（Web 经同路径 BFF），正文为 identifier、password 和可选 kind=phone/username；省略 kind 时服务端解析唯一标识，手机号与其他用户账号名冲突返回400，要求显式选择，不盲试另一账号。原 sign-in/username 与 sign-in/phone-number 保留为明确方式。大陆手机号统一为 +86 加11位号码；登录不套用新注册的密码长度限制。服务端在标准登录已被限流放行且正文校验通过之后读取账号凭据状态，再决定是否执行旧密码兼容校验，客户端不能选择算法或认证版本，也不存在“登录失败再试旧接口”的无差别降级。已证实的兼容算法为旧注册/改密链路的 `md5(password+salt)`（`legacy-md5-v1`）；`legacy-md5-v0` 仅作兼容保留，来源复核未在旧源码中找到写入路径；未知版本不猜测、不升级。兼容校验通过后在同一事务内写入当前密码哈希、清空旧 MD5 列并记录审计；升级失败（含审计写入失败）整体回滚且登录返回服务端错误；非法正文与限流请求不会触发升级。并发首次登录在账号行上串行，后到请求在旧凭据已被升级且当前哈希可校验时重放一次标准登录，不会永久卡在旧路径；升级成功但响应丢失后可直接重试。改密或找回成功后旧列由数据库触发器清除，旧密码不再可用；冻结、停用、注销账号不能借兼容入口建立会话；登录失败统一返回不区分账号存在性或新旧用户的错误。
...
`POST /api/auth/user/phone-registration/send-otp` 接收 phoneNumber；`POST /phone-registration/complete` 接收 phoneNumber、6位 code、password、acceptedTerms=true。验证码用途独立，有效5分钟，最多3次错误，每手机号发送冷却60秒；耗尽记录保留以约束冷却，允许重发时替换挑战。随机码可能重复数字，不把数字不同当作挑战身份保证。

完成注册时，验证码消费、用户、密码凭据及 UNVERIFIED/UNKNOWN 身份状态同事务提交。新密码遵循服务端长度政策（当前12–128位）；手机号验证不授予实名、年龄或交易资格。默认注册语义下已有手机号返回409且不改密码。显式传入 loginOrRegister=true 时，正确验证码可登录已有用户（仍执行停用/冻结会话校验）；新手机号未带password返回requiresPassword，不建用户/会话、不消费或延长挑战，补密码后完成注册。已有账号密码不会被该流程覆盖。账号提交后的会话建立失败或响应丢失应通过手机号密码登录恢复，不重复消费旧验证码。错误验证码400，发送限流429，未配置短信发送501；测试fixture使用假短信outbox，本地固定码模式见下文；真实短信未接入。
