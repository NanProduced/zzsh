...
完成注册时，验证码消费、用户、密码凭据及 UNVERIFIED/UNKNOWN 身份状态同事务提交。新密码遵循服务端长度政策（当前12–128位）；手机号验证不授予实名、年龄或交易资格。已有手机号返回409且不改密码。账号提交后的会话建立失败或响应丢失应通过手机号密码登录恢复，不重复消费旧验证码。错误验证码400，发送限流429，未配置短信发送501；当前仅测试fixture具备假短信outbox，正式服务不声称已发送。
